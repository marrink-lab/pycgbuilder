...
def make_edge(p0, p1, order, spacing=0.02, sep=0.25):
...
def draw_molecule(graph, clusters=None, labels=None, edge_widths=None, pos=None, ax=None):
    ...
    if edge_widths is None:
        widths = {0: 0, 1: 1, 1.5: 1.5, 2: 2, 3: 3, 4: 4}
    ...
    ax.add_collection(LineCollection(edges, color='black'))
    ax.add_collection(LineCollection(arom_edges, color='black', linestyle='--'))

    if clusters is not None:
        cmap = plt.get_cmap('tab10')
        for idx, position in pos.items():
            member = clusters[idx]
            ax.pie(member, center=position, radius=0.3,
